...
target2 = """                                    <span style={{ fontWeight: 600, color: '#334155' }}>
                                  ...
                                  {canModifyThisTicket ? (
                                    <select
                                      ...
                                      style={{ padding: '0.4rem 0.5rem', background: 'white', fontSize: '0.75rem', marginTop: '0.15rem' }}
                                      value={ticket.status}
                                      onChange={async (e) => {"""
...
replacement2 = """                                    <span style={{ fontWeight: 600, color: '#334155' }}>
                                  ...
                                  {canModifyThisTicket ? (
                                    <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                                      <select
                                        className="task-form-select"
                                        style={{ padding: '0.4rem 0.5rem', background: 'white', fontSize: '0.75rem', flex: 1 }}
                                        value={ticket.status}
                                        onChange={async (e) => {"""
...
replacement3 = """                                      <option value="in_review">In Review</option>
                                    ...
                                    <button 
                                      style={{ padding: '0.4rem 0.5rem', fontSize: '0.7rem', background: '#f1f5f9', border: '1px solid #cbd5e1', borderRadius: '4px', cursor: 'pointer' }}
                                      onClick={async () => {
...
print("App.jsx patched for Assignee Tickets Time Tracking (Second block)")
