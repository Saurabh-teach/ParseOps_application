...
target = """                              {planningData.suggested_due_date && (
                                <div style={{ fontSize: '0.775rem', color: '#475569' }}>
                                  Suggested due date (6.5h daily limit, no weekends): <strong style={{ color: '#4f46e5' }}>{new Date(planningData.suggested_due_date).toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })}</strong>
                                </div>
                              )}"""
