...
# Since replacing such a large block is prone to errors, we will replace the entire history method.

# Find the start of the history method
start_idx = content.find("    @action(detail=True, methods=['get'], url_path='history')")
# Find the end of the history method (next @action or end of class)
end_idx = content.find("    @action(detail=True, methods=['post'], url_path='update-member-status')", start_idx)
